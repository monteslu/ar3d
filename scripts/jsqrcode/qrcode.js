...
qrcode.decodeWithPoints = function(width, height, imageData){
    ...
    var text = qrcode.decode_utf8(str);
    if(text) {
      // make this similar to what window.BarcodeDetector returns
      var pts = qRCodeMatrix.points;
      return {
        rawValue: text,
        cornerPoints: [
          ...
          {x: pts[1].x, y: pts[1].y},
          {x: pts[2].x, y: pts[2].y},
          //infer bottom right point
          {x: pts[2].x - (pts[1].x - pts[0].x), y: pts[2].y + (pts[0].y - pts[1].y)},
          {x: pts[0].x, y: pts[0].y}
        ],
        js: true
      };
